"""Benchmark v5b: Monotone-Constrained MOAI.

V5 showed MOAI's epsilon oscillates due to noisy batch statistics.
Fix: constrain epsilon to be monotonically non-increasing.
This combines MOAI's closed-form optimality with AITI's stability.

Tests:
1. MOAI-Advantage (mono) vs MOAI-Advantage (free) vs AITI-Advantage vs PPO
2. MOAI-Entropy (mono) vs AITI-Entropy
3. Different EMA decays for monotone MOAI
"""
...
METHODS = [
    ...
    # Monotone MOAI variants
    ("moai_advantage_mono",  "MOAI-Adv mono (ema=0.95)", {
        "moai_ema_decay": 0.95, "moai_warmup_steps": 5}),
    ("moai_advantage_mono",  "MOAI-Adv mono (ema=0.90)", {
        "moai_ema_decay": 0.90, "moai_warmup_steps": 5}),
    ("moai_advantage_mono",  "MOAI-Adv mono (ema=0.80)", {
        "moai_ema_decay": 0.80, "moai_warmup_steps": 5}),
    # Free (oscillating) MOAI for comparison
    ("moai_advantage",       "MOAI-Adv free (ema=0.80)", {
        "moai_ema_decay": 0.80, "moai_warmup_steps": 5}),
    # Monotone entropy
    ("moai_entropy_mono",    "MOAI-Ent mono (ema=0.95)", {
        "moai_ema_decay": 0.95, "moai_warmup_steps": 5}),
]
...
def main():
    ...
    print(f"=== TI-PPO Benchmark v5b: Monotone-Constrained MOAI ===")
    print(f"Episodes: {args.episodes} | Methods: {len(methods_to_run)} | GPU: {device}")
    print(f"Hypothesis: Monotone constraint fixes epsilon oscillation\n")
    ...
    print(f"\n\n{'='*120}")
    print(f"{'BENCHMARK v5b RESULTS — MONOTONE-CONSTRAINED MOAI':^120}")
    print(f"{'='*120}")
    ...
    for label, h in histories.items():
        ...
        elif h.get("epsilon") and any(e != 1.0 for e in h["epsilon"]):
            n = len(h["epsilon"])
            q = max(1, n // 4)
            parts = []
            for qi in range(4):
