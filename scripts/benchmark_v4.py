"""Benchmark v4: Adaptive Intensity Token Importance (AITI).

KEY HYPOTHESIS: Token importance weighting introduces bias into PPO gradients.
The bias accumulates over training and eventually dominates the variance reduction benefit.
AITI solves this by DECAYING the importance intensity over training:
    w(t) = 1 + ε(step) · (s(t) - 1)
    ε decays from 1→0, transitioning from importance-weighted → uniform PPO.

Tests:
1. AITI variants with different inner scorers (entropy, advantage, adaptive_phase)
2. Different decay schedules (linear, quadratic, with/without residual)
3. Compared against PPO baseline and best v3 methods

If AITI beats PPO baseline on the Pareto frontier, it proves that:
- Token importance IS useful for PPO
- But only with proper bias-variance management
- Which no prior work has addressed
"""
...
METHODS = [
    # (method, triplet, label, extra_config)
    ("uniform",        False, "PPO baseline", {}),
    ("entropy",        False, "Entropy (no decay)", {}),
    ("adaptive_phase", False, "Adaptive Phase (no decay)", {}),
    # AITI with entropy inner scorer, different schedules
    ("aiti_entropy",   False, "AITI-Entropy (linear)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 120, "aiti_power": 1.0, "aiti_min_epsilon": 0.0}),
    ("aiti_entropy",   False, "AITI-Entropy (quadratic)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 120, "aiti_power": 2.0, "aiti_min_epsilon": 0.0}),
    ("aiti_entropy",   False, "AITI-Entropy (residual 0.1)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 100, "aiti_power": 1.0, "aiti_min_epsilon": 0.1}),
    # AITI with advantage inner scorer
    ("aiti_advantage",  False, "AITI-Advantage (linear)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 120, "aiti_power": 1.0, "aiti_min_epsilon": 0.0}),
    # AITI with adaptive phase inner scorer (best from v3)
    ("aiti_adaptive",   False, "AITI-AdaptivePhase (linear)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 120, "aiti_power": 1.0, "aiti_min_epsilon": 0.0,
        "phase_warmup_steps": 40}),
    ("aiti_adaptive",   False, "AITI-AdaptivePhase (quad)", {
        "aiti_epsilon_max": 1.0, "aiti_decay_steps": 120, "aiti_power": 2.0, "aiti_min_epsilon": 0.0,
        "phase_warmup_steps": 40}),
]
...
def run_method(method, use_triplet, label, extra_config, tokenizer, prompt_tokens, device, episodes=150):
    print(f"\n{'='*60}")
    print(f"  {label}")
    ...
    history = {"rewards": [], "kl": [], "importance": [], "epsilon": []}
    start = time.time()
    ...
    for ep in range(episodes):
        ...
        # Track epsilon for AITI methods
        if hasattr(trainer.scorer, 'epsilon'):
            history["epsilon"].append(trainer.scorer.epsilon)
        elif hasattr(trainer.scorer, 'step_count') and hasattr(trainer.scorer, 'warmup_steps'):
            alpha = min(1.0, trainer.scorer.step_count / trainer.scorer.warmup_steps)
            history["epsilon"].append(alpha)
        else:
        ...
        if ep % 25 == 0:
            eps_info = ""
            if hasattr(trainer.scorer, 'epsilon'):
                eps_info = f"  ε={trainer.scorer.epsilon:.3f}"
            print(f"  ep={ep:>3d}  r={stats['ppo/mean_reward']:.3f}  "
                  f"kl={stats['ppo/mean_kl']:.4f}  "
                  f"imp={stats['ti_ppo/mean_importance']:.3f}{eps_info}")
...
def main():
    ...
    print(f"=== TI-PPO Benchmark v4: Adaptive Intensity (AITI) ===")
    print(f"Episodes: {args.episodes} | Methods: {len(methods_to_run)} | GPU: {device}")
    print(f"Hypothesis: Decaying importance intensity solves bias-variance tradeoff\n")

    results = []
    histories = {}

    for method, triplet, label, extra in methods_to_run:
        r, h = run_method(method, triplet, label, extra, tokenizer, prompt_tokens,
    ...
    print(f"\n\n{'='*120}")
    print(f"{'BENCHMARK v4 RESULTS — ADAPTIVE INTENSITY':^120}")
    print(f"{'='*120}")
